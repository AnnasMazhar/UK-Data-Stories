...
st.set_page_config(page_title="Parliament - GovDataStory", page_icon="🏛️")
st.title("🏛️ Parliament")

story = get_latest_story("parliament")
if story:
    render_story_card(story)

st.markdown("### Key Insights")
insights = get_insights("parliament", limit=5)
if insights:
...
analysis = get_analysis("parliament")
ts = analysis.get("timeseries", {})
anom = analysis.get("anomalies", {})
if ts.get("months"):
    fig = build_trend_chart(ts["counts"], months=ts["months"], anomalies=anom.get("indices"), title="Parliament Dataset Publishing")
...
st.markdown("### Parliament Datasets")
datasets = get_datasets("parliament", limit=20)
if datasets:
