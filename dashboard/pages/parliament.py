...
if data.get('story'):
    st.markdown(f"""<div style="background:linear-gradient(135deg,#1e3c72,#2a5298);border-radius:12px;padding:20px;color:white"><h3>🏛️ {data['story']['headline']}</h3><p><b>Finding:</b> {data['story']['key_finding']}</p><p><b>Context:</b> {data['story']['context']}</p><p><b>Outlook:</b> {data['story']['outlook']}</p></div>""", unsafe_allow_html=True)
...
if data.get('months') and data.get('counts'):
    ...
    fig.update_layout(title="Parliament Publishing Over Time", template="plotly_white", height=350)
...
for d in get_datasets("parliament")[:10]:
