...
st.set_page_config(page_title="Population - GovDataStory", page_icon="👥")
st.title("👥 Population")

story = get_latest_story("population")
if story:
    render_story_card(story)

st.markdown("### Key Insights")
insights = get_insights("population", limit=5)
if insights:
...
analysis = get_analysis("population")
ts = analysis.get("timeseries", {})
anom = analysis.get("anomalies", {})
if ts.get("months"):
    fig = build_trend_chart(ts["counts"], months=ts["months"], anomalies=anom.get("indices"), title="Population Dataset Publishing")
...
st.markdown("### Population Datasets")
datasets = get_datasets("population", limit=20)
if datasets:
