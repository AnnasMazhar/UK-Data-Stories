...
st.set_page_config(page_title="Transport - GovDataStory", page_icon="🚂")
st.title("🚂 Transport")

story = get_latest_story("transport")
if story:
    render_story_card(story)

st.markdown("### Key Insights")
insights = get_insights("transport", limit=5)
if insights:
...
analysis = get_analysis("transport")
ts = analysis.get("timeseries", {})
anom = analysis.get("anomalies", {})
if ts.get("months"):
    fig = build_trend_chart(ts["counts"], months=ts["months"], anomalies=anom.get("indices"), title="Transport Dataset Publishing")
...
st.markdown("### Transport Datasets")
datasets = get_datasets("transport", limit=20)
if datasets:
