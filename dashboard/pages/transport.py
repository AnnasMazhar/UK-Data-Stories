"""Transport Page - UK Data Stories."""
...
st.set_page_config(page_title="Transport - UK Data Stories", page_icon="🚗")
...
st.title("🚗 Transport Data")
st.markdown("### UK Transport Dataset Intelligence")

data = get_topic_data("transport")

if data.get('story'):
    st.markdown(f"""<div style="background:linear-gradient(135deg,#1e3c72,#2a5298);border-radius:12px;padding:20px;color:white"><h3>🚗 {data['story']['headline']}</h3><p><strong>Finding:</strong> {data['story']['key_finding']}</p><p><strong>Context:</strong> {data['story']['context']}</p><p><strong>Outlook:</strong> {data['story']['outlook']}</p></div>""", unsafe_allow_html=True)
...
if data.get('months') and data.get('counts'):
    ...
    fig.update_layout(title="Transport Publishing",template="plotly_white",height=350); st.plotly_chart(fig, use_container_width=True)

st.markdown("### Sample Datasets")
for d in get_datasets("transport")[:10]: st.markdown(f"- **{d[0][:50]}** ({d[1]})")
st.caption("UK Data Stories | Transport")
