...
if data.get('months') and data.get('counts'):
    ...
    fig.update_layout(title="Housing Publishing",template="plotly_white",height=350); st.plotly_chart(fig, use_container_width=True)

st.markdown("### Sample Datasets")
for d in get_datasets("housing")[:10]: st.markdown(f"- **{d[0][:50]}** ({d[1]})")
