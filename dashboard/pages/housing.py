...
st.set_page_config(page_title="PAGE Title - UK Data Stories", page_icon="EMOJI")
...
st.title("EMOJI PAGE Title Data")
st.markdown("### UK PAGE Title Dataset Intelligence")

data = get_topic_data("PAGE")
...
if data.get('story'):
    ...
    <div style="background: linear-gradient(135deg, #1e3c72 0%, #2a5298 100%); border-radius: 12px; padding: 20px; margin: 12px 0; color: white;">
        <h3>EMOJI {data['story']['headline']}</h3>
...
if data.get('months') and data.get('counts'):
    ...
    fig.update_layout(title="PAGE Title Dataset Publishing Over Time", xaxis_title="Month", yaxis_title="Count", template="plotly_white", height=350)
    st.plotly_chart(fig, use_container_width=True)

st.markdown("### Sample Datasets")
for d in get_datasets("PAGE")[:10]:
    st.markdown(f"- **{d[0][:50]}...** ({d[1]})")

st.caption("UK Data Stories | PAGE Title")
