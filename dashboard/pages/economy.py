...
st.set_page_config(page_title="Economy - GovDataStory", page_icon="📊")
st.title("📊 Economy")

# Story
story = get_latest_story("economy")
if story:
    render_story_card(story)

# Insights
st.markdown("### Key Insights")
insights = get_insights("economy", limit=5)
if insights:
...
analysis = get_analysis("economy")
ts = analysis.get("timeseries", {})
anom = analysis.get("anomalies", {})
if ts.get("months"):
    fig = build_trend_chart(
        ts["counts"], months=ts["months"],
        anomalies=anom.get("indices"), title="Economy Dataset Publishing"
    )
...
# Data
st.markdown("### Economy Datasets")
datasets = get_datasets("economy", limit=20)
if datasets:
