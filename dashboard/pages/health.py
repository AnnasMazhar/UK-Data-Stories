"""Health Page - UK Data Stories."""
import streamlit as st
import plotly.graph_objects as go
import duckdb, json
st.set_page_config(page_title="Health - UK Data Stories", page_icon="🏥")
...
st.markdown("### UK Health Dataset Intelligence")
data = get_topic_data("health")
if data.get('story'):
...
if data.get('months') and data.get('counts'):
    ...
    fig.update_layout(title="Health Publishing",template="plotly_white",height=350); st.plotly_chart(fig,use_container_width=True)
st.markdown("### Sample Datasets")
for d in get_datasets("health")[:10]: st.markdown(f"- **{d[0][:50]}...** ({d[1]})")
st.caption("UK Data Stories | Health")
