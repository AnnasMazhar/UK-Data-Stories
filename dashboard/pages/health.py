...
st.set_page_config(page_title="Health - GovDataStory", page_icon="🏥")
st.title("🏥 Health")

story = get_latest_story("health")
if story:
    render_story_card(story)

st.markdown("### Key Insights")
insights = get_insights("health", limit=5)
if insights:
...
analysis = get_analysis("health")
ts = analysis.get("timeseries", {})
anom = analysis.get("anomalies", {})
if ts.get("months"):
    fig = build_trend_chart(ts["counts"], months=ts["months"], anomalies=anom.get("indices"), title="Health Dataset Publishing")
...
st.markdown("### Health Datasets")
datasets = get_datasets("health", limit=20)
if datasets:
